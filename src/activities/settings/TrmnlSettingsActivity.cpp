...
// items are reordered or new ones inserted before it -- the other indices (0-3) are
// pre-existing and unchanged by this feature, left as-is to keep this change scoped.
constexpr int MENU_INDEX_REFRESH_ON_POWER_BUTTON = 4;
...
void TrmnlSettingsActivity::handleSelection() {
  if (selectedIndex == 0) {
    ...
    SETTINGS.saveToFile();
    requestUpdate();
  } else if (selectedIndex == MENU_INDEX_REFRESH_ON_POWER_BUTTON) {
